...
class VolSDFNetworkBG(nn.Module):
    ...
    def __init__(self, conf):
        ...
        self.implicit_network = ImplicitNetwork(
            self.feature_vector_size, 0.0,
            **conf.get_config('implicit_network'))
        self.rendering_network = RenderingNetwork(
            self.feature_vector_size, **conf.get_config('rendering_network'))

        self.density = LaplaceDensity(**conf.get_config('density'))
        self.ray_sampler = ErrorBoundSampler(self.scene_bounding_sphere,
        ...
        self.bg_implicit_network = ImplicitNetwork(
            bg_feature_vector_size, 0.0,
            **conf.get_config('bg_network.implicit_network'))
        self.bg_rendering_network = RenderingNetwork(
            bg_feature_vector_size,
            **conf.get_config('bg_network.rendering_network'))
        self.bg_density = AbsDensity(
            **conf.get_config('bg_network.density', default={}))
    ...
    def forward(self, input):
        ...
        sdf, feature_vectors, gradients = self.implicit_network.get_outputs(
            points_flat)

        rgb_flat = self.rendering_network(points_flat, gradients, dirs_flat,
                                          feature_vectors)
        ...
        # Rendered depth
        fg_depth_values = torch.sum(weights * z_vals, 1)

        # Rendered normal
        fg_normals = gradients / gradients.norm(2, -1, keepdim=True)
        fg_normals = fg_normals.reshape(-1, N_samples, 3)
        fg_normal_values = torch.sum(weights.unsqueeze(-1) * fg_normals, 1)

        # ----------------------------------
        # Background rendering
        N_bg_samples = z_vals_bg.shape[1]
        z_vals_bg = torch.flip(z_vals_bg, dims=[
            -1,
        ])  # 1--->0
        ...
        bg_points = self.depth2pts_outside(bg_locs, bg_dirs,
                                           z_vals_bg)  # [..., N_samples, 4]
        ...
        bg_rgb_flat = self.bg_rendering_network(None, None, bg_dirs_flat,
                                                bg_feature_vectors)
        ...
        if self.training:
            ...
            eikonal_points = torch.empty(n_eik_points, 3).uniform_(
            ...
            eik_near_points = (
                cam_loc.unsqueeze(1) +
                z_samples_eik.unsqueeze(2) * ray_dirs.unsqueeze(1)).reshape(
                    -1, 3)
    ...
    def volume_rendering(self, z_vals, z_max, sdf):
        ...
        density = density_flat.reshape(
            -1, z_vals.shape[1])  # (batch_size * num_pixels) x N_samples
        ...
        shifted_free_energy = torch.cat(
            [torch.zeros(dists.shape[0], 1).cuda(), free_energy],
            dim=-1)  # add 0 for transperancy 1 at t_0
        alpha = 1 - torch.exp(
            -free_energy)  # probability of it is not empty here
        ...
        bg_transmittance = transmittance[:,
                                         -1]  # factor to be multiplied with the bg volume rendering
    ...
    def bg_volume_rendering(self, z_vals_bg, bg_sdf):
        ...
        bg_density = bg_density_flat.reshape(
            -1, z_vals_bg.shape[1])  # (batch_size * num_pixels) x N_samples

        bg_dists = z_vals_bg[:, :-1] - z_vals_bg[:, 1:]
        bg_dists = torch.cat([
            bg_dists,
            torch.tensor([1e10]).cuda().unsqueeze(0).repeat(
                bg_dists.shape[0], 1)
        ], -1)
        ...
        bg_shifted_free_energy = torch.cat(
            [torch.zeros(bg_dists.shape[0], 1).cuda(), bg_free_energy[:, :-1]],
            dim=-1)  # shift one step
        bg_alpha = 1 - torch.exp(
            -bg_free_energy)  # probability of it is not empty here
    ...
    def depth2pts_outside(self, ray_o, ray_d, depth):
        ...
        under_sqrt = o_dot_d**2 - (
            (ray_o**2).sum(-1) - self.scene_bounding_sphere**2)
