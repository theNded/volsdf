...
class ImplicitNetwork(nn.Module):
    ...
    def get_outputs(self, x):
        ...
        if self.sdf_bounding_sphere > 0.0:
            sphere_sdf = self.sphere_scale * (self.sdf_bounding_sphere -
...
class VolSDFNetwork(nn.Module):
    ...
    def __init__(self, conf):
        ...
        self.bg_color = torch.tensor(
            conf.get_list("bg_color", default=[1.0, 1.0, 1.0])).float().cuda()
    ...
    def volume_rendering(self, z_vals, sdf):
        ...
        shifted_free_energy = torch.cat(
            [torch.zeros(dists.shape[0], 1).cuda(), free_energy[:, :-1]],
